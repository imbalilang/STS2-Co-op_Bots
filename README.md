...
## 最新更新 / Latest：0.37.1（2026-09-22）

- **修复混合局里真人选牌被 Bot 顶掉**：桌上一旦有席位交给 AI，真人自己的选牌（生存者弃牌、攻击／能力药水三选一、事件选牌）会被 Bot 直接答掉且不同步给其他玩家。现在只有"全部席位都交给 AI"时才启用那套自动选择钩子。
- 上一版（0.37.0）：**全 Bot 模式接管战斗**——每个席位都交给 AI 时，开局一次长搜索（基础 3 分钟，续跑整场上限 8 分钟），之后严格按剧本打完；一段结束再从实盘算下一段。桌上还有真人时一切照旧。
- 上一版（0.37.0）：执行侧三处修复（席位阵亡不丢整条剧本／回合跨越校验覆盖所有步骤／剧本在飞时旧规划器沉默）、未建模机制不进剧本、寻路回避当前幕造不出的房间。
...
**已发布版本 0.37.1**，安装包 `outputs/CoopBots-v0.37.1.zip`。运行目标为游戏 v0.111.0，
