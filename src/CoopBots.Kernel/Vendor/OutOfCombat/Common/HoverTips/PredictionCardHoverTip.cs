...
{
    /// <summary>
    /// Local addition for the kernel facade. Upstream only ever renders this tip,
    /// so the card is otherwise reachable only through the base HoverTip; the
    /// kernel needs the model itself.
    /// </summary>
    public CardModel PredictedCard { get; } = card;
