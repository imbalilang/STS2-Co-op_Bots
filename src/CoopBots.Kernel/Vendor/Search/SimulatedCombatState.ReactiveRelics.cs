...
internal sealed partial class SimulatedCombatState
{
        ...
        int etherealExhaustCount, IReadOnlyDictionary<Creature, int>? etherealByOwner = null)
    {
                     ...
                     .Where(relic => !relic.IsMelted && participants.Contains(relic.Owner.Creature)))
        {
            switch (relic)
            {
                case JossPaper value when (etherealByOwner?.GetValueOrDefault(value.Owner.Creature) ?? etherealExhaustCount) > 0:
                {
                    ...
                    int exhausted = RelicPredictionStateSupport.GetJossPaperCardsExhausted(simulator, value)
                        + (etherealByOwner?.GetValueOrDefault(value.Owner.Creature) ?? etherealExhaustCount);
